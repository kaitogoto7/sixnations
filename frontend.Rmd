...
full.time.margin <- function(nation){
  ...
  away.margin <- sum(away.df[,1])/10 - sum(away.df[,2])/10
  home.df <- read.csv(paste0("./6nationsHomeAdvantages/", filenames[indexes[2]]))[,-c(1)] #home csv is read
  home.margin <- sum(home.df[,1])/10 - sum(home.df[,2])/10
...
half.time.margin <- function(nation){
  ...
  indexes <- grep(nation, filenames)
  away.df <- read.csv(paste0("./6nationsHomeAdvantages/", filenames[indexes[1]]))[,-c(1)] #away csv is read
  away.margin <- sum(away.df[,3])/10 - sum(away.df[,4])/10
  home.df <- read.csv(paste0("./6nationsHomeAdvantages/", filenames[indexes[2]]))[,-c(1)] #home csv is read
  home.margin <- sum(home.df[,3])/10 - sum(home.df[,4])/10
  return(home.margin - away.margin)
}
...
layout <- list(
  ...
  width = 750, 
  xaxis = list(
    #categoryarraysrc = "kaitogoto7:2:933abe", 
    categoryorder = "array", 
    domain = c(0, 1), 
    title = "Metric", 
    type = "category"
